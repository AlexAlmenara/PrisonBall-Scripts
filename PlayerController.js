#pragma strict
/* Is almost ThirdPersonController.js, but it had been modified:
- 
*/
...
/*
function HidePlayer() //XXX: check with Disappear in BallPlayer.js
{
...
{
	var cameraTransform = playerCamera.transform; //Camera.main.transform;
...
function ApplyGravity ()
{
	...
	{
		// Apply gravity
		var jumpButton = Input.GetButton(key_jump);
...
function SetKeyGamer1() {
	...
	playerCamera.SendMessage("SetKeyCenter", key_center);
}
...
function PlayerBall() {
	//si pulsa tecla Fire2, intenta coger pelota
	
	//if (Input.GetButton(key_center))
		//playerCamera.SendMessage("Center");
	
		
	//if (!s_BallPlayer.IsBusy() && Input.GetButtonUp(key_fire) /*&& controller.IsGroundedWithTimeout()*/ && !IsMoving()) { //once free the key: catch or throw
	if (!s_BallPlayer.IsBusy() && !IsMoving()) {
		...
		if (Input.GetButtonUp(key_fire)) { //Catch or throw ball
			if ((!s_BallPlayer.HasBall()) && (!s_BallControl.IsCaught())) //if hasn't the ball and nobody has the ball, try catch it
				...
				//if(!s_BallPlayer.IsBusy() && Input.GetButtonDown("Fire2") && !IsMoving()) //puede hacer movimiento de tirar sin pelota
				gameObject.SendMessage("TryThrowBall", keyThrowPower);
			...
			if (Input.GetButtonUp(key_pass)) {
				//print("key_pass of player " + s_BallPlayer.GetName());
	...
	if (s_BallPlayer.HasBall()) {
		...
		s_BallPlayer.AimBall(v, h); //apuntar con la pelota. XXX: maybe quit this function, only follow hand
	}
}
...
function Update() {
	...
	else //by alex: if jump he can't aim the ball
		PlayerBall(); //control the ball
...
{
	return Mathf.Abs(Input.GetAxisRaw(key_vertical)) + Mathf.Abs(Input.GetAxisRaw(key_horizontal)) > 0.5;
}
